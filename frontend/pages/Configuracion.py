...
def generate_bookmarklet():
    js_code = """(function(){
        ...
        function updateImagePreview(){
            ...
            preview.innerHTML=Array.from(selectedImages).map(function(url){
                return '<img src="'+url+'" style="height:50px;margin:2px;border:1px solid #ccc;">';
            }).join('');
            document.getElementById('selectedCount').textContent=selectedImages.size;
        }
        d.innerHTML='<h3 style="margin:0 0 15px">Guardar Propiedad</h3><form id="propertyForm" style="display:flex;flex-direction:column;gap:15px"><div><label>URL:</label><input type="text" name="url" value="'+window.location.href+'" readonly style="width:100%;padding:5px"></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><div><label>Precio:</label><input type="text" name="price" required style="width:100%;padding:5px"></div><div><label>Ubicación:</label><input type="text" name="location" required style="width:100%;padding:5px"></div></div><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px"><div><label>Habitaciones:</label><input type="number" name="rooms" min="0" style="width:100%;padding:5px"></div><div><label>Baños:</label><input type="number" name="bathrooms" min="0" step="0.5" style="width:100%;padding:5px"></div><div><label>Niveles:</label><input type="number" name="levels" min="0" style="width:100%;padding:5px"></div></div><div><label>Descripción:</label><textarea name="description" required style="width:100%;height:80px;padding:5px"></textarea></div><div><label>Datos del vendedor:</label><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><input type="text" name="agent_name" placeholder="Nombre" style="width:100%;padding:5px"><input type="text" name="agent_phone" placeholder="Teléfono" style="width:100%;padding:5px"></div><input type="email" name="agent_email" placeholder="Email" style="width:100%;padding:5px;margin-top:5px"></div><div><label>Imágenes: <span id="selectedCount">0</span> seleccionadas</label><button type="button" onclick="window.enableImageSelection()" style="width:100%;padding:10px;margin:5px 0;background:#2196F3;color:white;border:none;border-radius:4px;cursor:pointer">Seleccionar imágenes</button><div id="imagePreview" style="max-height:100px;overflow-y:auto;padding:5px;border:1px solid #ccc;margin:5px 0;display:flex;flex-wrap:wrap;gap:5px"></div></div><div style="display:flex;gap:10px;justify-content:flex-end;margin-top:10px"><button type="submit" style="padding:10px 20px;background:#4CAF50;color:white;border:none;border-radius:4px;cursor:pointer">Guardar</button><button type="button" onclick="this.closest(\'div\').parentElement.parentElement.remove()" style="padding:10px 20px;background:#f44336;color:white;border:none;border-radius:4px;cursor:pointer">Cancelar</button></div></form>';
